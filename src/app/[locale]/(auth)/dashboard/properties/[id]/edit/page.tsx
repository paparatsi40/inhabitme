    ...
    const fetchListing = async () => {
      try {
        ...
        setFormData({
          ...
          city: listing?.city_name || listing?.city?.name || '',
          neighborhood: typeof listing?.neighborhood === 'string'
            ? listing.neighborhood
            : listing?.neighborhood?.name || '',
          hasDesk: listing?.has_desk ?? listing?.amenities?.hasDesk ?? false,
          wifiSpeed: listing?.wifi_speed_mbps ?? listing?.amenities?.wifiSpeedMbps ?? 0,
          hasSecondMonitor: listing?.has_second_monitor ?? listing?.amenities?.hasSecondMonitor ?? false,
          furnished: listing?.furnished ?? listing?.amenities?.furnished ?? false,
          hasHeating: listing?.has_heating || false,
          hasAc: listing?.has_ac || false,
          hasBalcony: listing?.has_balcony || false,
          hasTerrace: listing?.has_terrace || false,
          hasWashingMachine: listing?.has_washing_machine || false,
          hasDryer: listing?.has_dryer || false,
          hasDishwasher: listing?.has_dishwasher || false,
          hasKitchen: listing?.has_kitchen || false,
          hasElevator: listing?.has_elevator || false,
          hasParking: listing?.has_parking || false,
          hasDoorman: listing?.has_doorman || false,
          ...
          hasSecuritySystem: listing?.has_security_system || false,
          hasSafe: listing?.has_safe || false,
          monthlyPrice: listing?.monthly_price ?? listing?.price?.monthly ?? 1000,
          minStayMonths: listing?.min_months ?? listing?.availability?.minMonths ?? 1,
          maxStayMonths: listing?.max_months ?? listing?.availability?.maxMonths ?? 12,
        })
      } catch (err: any) {
